...
export default function SplashScreen() {
  ...
  useEffect(() => {
    const t1 = setTimeout(() => setFadeOut(true), DURATION_MS);
    const t2 = setTimeout(() => setVisible(false), DURATION_MS + FADE_OUT_MS);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
    };
  ...
  return (
    ...
    >
      {/* Fundo: branco → preto no mesmo tempo do bigode */}
      <div
        className="absolute inset-0 bg-white"
        style={{
          animation: "splash-bg-to-black 2.2s ease-out forwards",
        }}
      />
      {/* Bigode: centro, bem pequeno → cresce */}
      <div
        className="absolute inset-0 flex items-center justify-center"
        style={{
          animation: "splash-mustache-grow 2.2s ease-out forwards",
        }}
      >
        <svg
