...
export async function GET(
  ...
  await connectDB();
  const doc = await FinancialRecord.findOne({ _id: id, tenantId: auth.tenantId }).lean();
...
export async function PUT(
  ...
  if (body.recurrence != null && ["unique", "weekly", "biweekly", "monthly"].includes(body.recurrence)) {
    update.recurrence = body.recurrence;
  }
  if (typeof body.isFixedExpense === "boolean") update.isFixedExpense = body.isFixedExpense;
...
export async function DELETE(
  ...
  await connectDB();
  const result = await FinancialRecord.deleteOne({ _id: id, tenantId: auth.tenantId });
